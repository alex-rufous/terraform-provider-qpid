...
func createUser(d *schema.ResourceData, meta interface{}) error {
	...
	parent := d.Get("authentication_provider").(string)
	resp, err := client.CreateUser(parent, attributes)
	if err != nil {
		return err
	}
	...
	if resp.StatusCode == http.StatusCreated {
		...
		if err != nil {
			var err2 error
			attributes, err2 = client.GetUser(parent, name)
			if err2 != nil {
				return err
			}
		}
	...
	return fmt.Errorf("error creating qpid user '%s/%s': %s", parent, name, resp.Status)
}
...
func readUser(d *schema.ResourceData, meta interface{}) error {
	...
	attributes, err := client.GetUser(parent, name)
	if err != nil {
		return err
	}
	if len(*attributes) == 0 {
...
func deleteUser(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := client.DeleteUser(parent, name)
	if err != nil {
		return nil
	}

	if resp.StatusCode >= http.StatusBadRequest {
		return fmt.Errorf("error deleting qpid user '%s' on node %s: %s", name, parent, resp.Status)
	}
	d.SetId("")
	return nil
}
...
func updateUser(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := client.UpdateUser(parent, name, attributes)
	if err != nil {
		return err
	}
	...
	return fmt.Errorf("error updating qpid user '%s' on node '%s': %s", name, parent, resp.Status)
}
